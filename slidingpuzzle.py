...
class Card:
    ...
    def place_tile(self, xpos, ypos):
        ...
        # Create the image on the rectangle
...
def create_cards(main_image_path):
    ...
    slice_width = main_image.width // 10
    slice_height = main_image.height // 10

    # Prepare all image slices
    for y in range(10):
        for x in range(10):
            if x == 9 and y == 9:  # create empty slot (empty slot)
                continue
    ...
    for y in range(10):
        for x in range(10):
            if x == 9 and y == 9:  # Skip the last tile (empty slot)
                continue
...
def reset_game():
    ...
    all_cards=[]
    all_images.clear()
    empty_slot=(9,9) # need to restart empty slot 
